...
# make_data_hasuda05 Copy
machine=make_data_hasuda05
if [ ! -e ${devdir}/make_data_hasuda05/${machine} ]; then   # 
コンパイル済みか？（実行ファイルが存在する？）
echo "${machine} ${devdir}/make_data_hasuda05/ Go Build!"
    cd ${devdir}/make_data_hasuda05/
    go build .
else
    echo ""
...
echo "${devdir}/make_data_hasuda05/* --> ${deptop}/${machine}/"
cp -rfp ${devdir}/make_data_hasuda05/* ${deptop}/${machine}/                  # files copy
#mv ${deptop}/${machine}/make_data_hasuda05 ${deptop}/${machine}/${machine}    # change bin name
